...
| `intelligence_behavior_authority`         | Optional mapping of fixture entity IDs to learned power-state authority: `brightness_only` never proposes learned on/off actions, `on_off` permits both, and `off_only` permits only learned turn-off. Unmapped discovered fixtures default to `on_off`; invalid values fail closed and are rejected.                                                                                             | `{}`           | `dict`                                  |
